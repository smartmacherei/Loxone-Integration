...
SENSOR_TYPES: tuple[LoxoneEntityDescription, ...] = (
    ...
    LoxoneEntityDescription(
        # Loxone gibt Temperaturen oft nur mit "°" (statt "°C") aus. Da "°" auch
        # fuer Winkel verwendet wird, greift dieser Fall NUR bei einem
        # Temperatur-Schluesselwort in Name/Kategorie.
        key="temperature_deg",
        loxone_format_strings=("°",),
        name_keywords=("temp", "temperatur", "temperature", "teplota"),
        category_keywords=("temp", "temperatur", "temperature", "klima", "climate"),
        state_class=SensorStateClass.MEASUREMENT,
        device_class=SensorDeviceClass.TEMPERATURE,
    ),
    LoxoneEntityDescription(
...
def match_sensor_description(
    ...
    for desc in SENSOR_TYPES:
        if unit_lower not in {u.lower() for u in desc.loxone_format_strings}:
            continue
        if not desc.category_keywords and not desc.name_keywords:
            return desc
...
class LoxoneSensor(LoxoneEntity, SensorEntity):
    ...
    def __init__(self, **kwargs):
        ...
        if desc:
            ...
            # Loxone-Rohunits auf HA-kanonische Einheiten bringen, sonst meckert
            # HA bei device_class ueber ungueltige Einheiten ("°"/"Lx").
            unit = self._attr_native_unit_of_measurement
            if desc.device_class == SensorDeviceClass.TEMPERATURE and unit == "°":
                self._attr_native_unit_of_measurement = UnitOfTemperature.CELSIUS
            elif desc.device_class == SensorDeviceClass.ILLUMINANCE and (unit or "").lower() in ("lx", "lux"):
    ...
    async def async_added_to_hass(self):
        ...
        # Auto-entdeckte Klemmen, deren Wert der Miniserver nicht ueber den
        # WS-Stream pusht, mit dem per HTTP geholten Initialwert seeden.
        if self._attr_native_value is None:
            from .helpers import initial_values

            if self.uuidAction in initial_values:
                self._attr_native_value = self._get_lox_rounded_value(
                    initial_values[self.uuidAction]
                )
                self.async_schedule_update_ha_state()
